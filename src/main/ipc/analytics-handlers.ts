...
import { aggregateDailySales, runIncrementalAggregation } from '../services/analytics/aggregation'
import {
  getDailyMetrics,
  getPeriodMetrics,
  getTopProducts,
...
export function registerAnalyticsHandlers(): void {
  ...
  ipcMain.handle(
    ...
    (
      _event,
      {
        ...
        endDate,
        limit
      }: { userId: string; startDate: string; endDate: string; limit?: number }
    ) => {
  ...
  ipcMain.handle(
    IPC_CHANNELS.ANALYTICS_LOW_STOCK_ALERTS,
    (_event, { userId }: { userId: string }) => {
  ...
  ipcMain.handle(
    IPC_CHANNELS.ANALYTICS_INVENTORY_VALUATION,
    (_event, { userId }: { userId: string }) => {
